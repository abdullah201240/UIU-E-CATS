...
<nav class="navbar">
    <div class="navbar-left">
        ...
        <div class="search-box">
            ...
            <input type="text" placeholder="Search by name or basic info" id="searchInput">
        </div>
    </div>
    ...
    @foreach ($data1 as $da1)
        <div class="navbar-right">
            <div class="online">
                <img src="images/{{ $da1->image }}" class="nav-profile-img" onclick="toggleMenu()">
            </div>
        </div>
        <!--profile drop menu-->
        <div class="profile-menu-wrap" id="profileMenu">
            <div class="profile-menu">
                <div class="user-info">
                    <img src="images/{{ $da1->image }}">
                    <div>
                        <h3>{{ $da1->name }}</h3>
                        <a href="adminprofile">See Your Profile</a>
                    </div>
                </div>
                <hr>
...
<div class="container1">
    <center>
        <h1 style="color:#38406e;">Alumni List</h1>
        <hr style="color:#38406e;">
    </center> <br>
    <div class="box-container">
        @foreach ($data as $da)
            <div class="box">
                <div class="image">
                    <img src="images/{{ $da->image }}" alt="">
                </div>
                <div class="content">
                    <h3>{{ $da->name }}</h3>
                    <p>Phone Number : {{ $da->phone }} Email: {{ $da->email }}</p>
                    ...
                    <a href="connectallumni/{{ $da->name }}/{{ $da->image }}/{{ $da->id }}/{{ $da->email}}"
                        class="btn">Connect</a>
                    <div class="icons">
                    </div>
                </div>
...
<script>
    ...
    // listen for changes in the search input value
    searchInput.addEventListener('input', () => {
        const searchText = searchInput.value.toLowerCase();

        // loop through all the box elements
        boxes.forEach(box => {
            const name = box.querySelector('h3').textContent.toLowerCase();
            const basicInfo = box.querySelector('p').textContent.toLowerCase();

            // check if the name or basic info contains the search text
            if (name.includes(searchText) || basicInfo.includes(searchText)) {
